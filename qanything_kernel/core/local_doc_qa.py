...
from qanything_kernel.connector.embedding.embedding_for_online_client import (
    YouDaoEmbeddings,
)
...
class LocalDocQA:
    def __init__(self, port):
        ...
        self.doc_splitter = CharacterTextSplitter(
            chunk_size=LOCAL_EMBED_MAX_LENGTH / 2, chunk_overlap=0, length_function=len
        )
    ...
    def init_cfg(self, args=None):
        ...
        self.retriever = ParentRetriever(
            self.milvus_kb, self.milvus_summary, self.es_client
        )
    ...
    def get_web_search(self, queries, top_k):
        ...
        for idx, doc in enumerate(web_documents):
            ...
            if "description" in doc.metadata:
                desc_doc = Document(
                    page_content=doc.metadata["description"], metadata=doc.metadata
                )
    ...
    def reprocess_source_documents(
        ...
        history_token_num = int(
            custom_llm.num_tokens_from_messages(
                [x for sublist in history for x in sublist]
            )
        )
        template_token_num = int(custom_llm.num_tokens_from_messages([prompt_template]))

        reference_field_token_num = int(
            custom_llm.num_tokens_from_messages(
                [
                    f"<reference>[{idx + 1}]</reference>"
                    for idx in range(len(source_docs))
                ]
            )
        )
        limited_token_nums = (
        ...
        """.format(
            ...
            template_token_num=template_token_num,
            reference_field_token_num=reference_field_token_num,
            query_token_num=query_token_num // 4,
            ...
            for doc in source_docs:
                ...
                if file_id not in not_repeated_file_ids:
                    ...
                    if "headers" in doc.metadata:
                        headers = f"headers={doc.metadata['headers']}"
                        context += (
                            f"<reference {headers}>[{len(not_repeated_file_ids)}]"
                            + "\n"
                            + doc_valid_content
                            + "\n"
                        )
                        ...
                        context += (
                            f"<reference>[{len(not_repeated_file_ids)}]"
                            + "\n"
                            + doc_valid_content
                            + "\n"
                        )
    ...
    async def get_rerank_results(self, query, doc_ids=None, doc_strs=None):
        ...
        if doc_strs:
            ...
            for doc_id in doc_ids:
                ...
                if doc_json is None:
                    docs.append(None)
                    continue
                user_id, file_id, file_name, kb_id = (
                    doc_json["kwargs"]["metadata"]["user_id"],
                    doc_json["kwargs"]["metadata"]["file_id"],
                    doc_json["kwargs"]["metadata"]["file_name"],
        ...
        if len(docs) > 1 and num_tokens_rerank(query) <= 300:
            try:
                ...
                debug_logger.error(
                    f"query tokens: {num_tokens_rerank(query)}, rerank error: {e}"
                )
    ...
    async def prepare_source_documents(
        self,
        custom_llm: OpenAILLM,
            ...
            if new_docs:
                ...
                for file_id in merged_documents_file_ids:
                    docs = [
                        doc
                        for doc in retrieval_documents
                        if doc.metadata["file_id"] == file_id
                    ]
                    docs = sorted(
                        docs, key=lambda x: int(x.metadata["doc_id"].split("_")[-1])
                    )
    ...
    async def calculate_relevance_optimized(
        ...
        for doc_index in indices[0]:
            doc_id = doc_index // len(
                self.doc_splitter.split_documents([reference_docs[0]])
            )
            ...
            similarity_llm = 1 - cosine(
                llm_answer_embedding, reference_embeddings[doc_index]
            )
            rerank_score = question_scores[doc_id]

            # 设置rerank分数和LLM回答与文档余弦相似度的权重
            weights = [0.5, 0.5]  # 分别对应similarity_llm和rerank_score
            combined_score = weighted_geometric_mean(
                [similarity_llm, rerank_score], weights
            )
    ...
    async def get_knowledge_based_answer(
        ...
        custom_llm = OpenAILLM(
            model, max_token, api_base, api_key, api_context_length, top_p, temperature
        )
        ...
        if chat_history:
            ...
            full_prompt = rewrite_q_chain.condense_q_prompt.format(
                chat_history=formatted_chat_history, question=query
            )
            ...
            try:
                ...
                time_record["rewrite_completion_tokens"] = (
                    custom_llm.num_tokens_from_messages([condense_question])
                )
                debug_logger.info(
        ...
        if kb_ids:
            source_documents = await self.get_source_documents(
                retrieval_query, retriever, kb_ids, time_record, hybrid_search, top_k
            )
        else:
            source_documents = []
        ...
        if need_web_search:
            ...
            for doc in web_search_results:
                ...
                self.milvus_summary.add_document(
                    doc_id=doc.metadata["doc_id"], json_data=doc_json
                )
        ...
        if rerank and len(source_documents) > 1 and num_tokens_rerank(query) <= 300:
            try:
                ...
                source_documents = await self.rerank.arerank_documents(
                    condense_question, source_documents
                )
                t2 = time.perf_counter()
                ...
                if len(source_documents) > 1:
                    if filtered_documents := [
                        doc for doc in source_documents if doc.metadata["score"] >= 0.28
                    ]:
                    ...
                    for doc in source_documents[1:]:
                        ...
                        relative_difference = (
                            saved_docs[0].metadata["score"] - doc.metadata["score"]
                        ) / saved_docs[0].metadata["score"]
        ...
        high_score_faq_documents = [
            doc
            for doc in source_documents
            if doc.metadata["file_name"].endswith(".faq")
            and doc.metadata["score"] >= 0.9
        ]
        ...
        if source_documents:
            if custom_prompt:
                ...
                system_prompt = SYSTEM.replace("{{today_date}}", today).replace(
                    "{{current_time}}", now
                )
            ...
            t1 = time.perf_counter()
            retrieval_documents, limited_token_nums, tokens_msg = (
                self.reprocess_source_documents(
                    custom_llm=custom_llm,
                    query=query,
            ...
            if custom_prompt:
                ...
                prompt_template = (
                    SIMPLE_PROMPT_TEMPLATE.replace("{{today}}", today)
                    .replace("{{now}}", now)
                ...
                prompt_template = (
                    SIMPLE_PROMPT_TEMPLATE.replace("{{today}}", today)
                    .replace("{{now}}", now)
                    .replace("{{custom_prompt}}", simple_custom_prompt)
                )
        ...
        prompt = self.generate_prompt(
            query=query, source_docs=source_documents, prompt_template=prompt_template
        )
        # debug_logger.info(f"prompt: {prompt}")
        est_prompt_tokens = num_tokens(prompt) + num_tokens(str(chat_history))
        async for answer_result in custom_llm.generatorAnswer(
            ...
            time_record["prompt_tokens"] = (
                prompt_tokens if prompt_tokens != 0 else est_prompt_tokens
            )
            time_record["completion_tokens"] = (
                completion_tokens if completion_tokens != 0 else num_tokens(acc_resp)
            )
            time_record["total_tokens"] = (
                total_tokens
                if total_tokens != 0
                else time_record["prompt_tokens"] + time_record["completion_tokens"]
            )
            ...
            if resp[6:].startswith("[DONE]"):
                ...
                time_record["llm_completed"] = (
                    round(last_return_time - t1, 2) - time_record["llm_first_return"]
                )
    ...
    def get_completed_document(self, file_id, limit=None):
        ...
        for doc_json in sorted_json_datas:
            ...
            completed_content += (
                re.sub(r"!\[figure]\(.*?\)", "", doc.page_content) + "\n\n"
            )  # 删除图片
        completed_doc_with_figure = Document(
            page_content=completed_content_with_figure,
            metadata=sorted_json_datas[0]["kwargs"]["metadata"],
        )
        completed_doc = Document(
            page_content=completed_content,
            metadata=sorted_json_datas[0]["kwargs"]["metadata"],
        )
        # FIX metadata
        has_table = False
    ...
    def aggregate_documents(
        self, source_documents, limited_token_nums, custom_llm, rerank
    ):
        ...
        for doc in source_documents:
            ...
            if not first_file_dict:
                ...
                first_file_dict["doc_ids"] = [
                    int(doc.metadata["doc_id"].split("_")[-1])
                ]
                ...
                second_file_dict["doc_ids"].append(
                    int(doc.metadata["doc_id"].split("_")[-1])
                )
                ori_second_docs.append(doc)
            else:  # 如果有第三个文件，直接返回
        ...
        new_docs = []
        first_completed_doc, first_completed_doc_with_figure = (
            self.get_completed_document(first_file_dict["file_id"])
        )
        ...
        if first_doc_tokens + ori_second_docs_tokens > limited_token_nums:
            ...
            doc_limit = [
                min(first_file_dict["doc_ids"]),
                max(first_file_dict["doc_ids"]),
            ]
            first_completed_doc_limit, first_completed_doc_limit_with_figure = (
                self.get_completed_document(first_file_dict["file_id"], doc_limit)
            )
            first_completed_doc_limit.metadata["score"] = first_file_dict["score"]
            first_doc_tokens = custom_llm.num_tokens_from_docs(
                [first_completed_doc_limit]
            )
            if first_doc_tokens + ori_second_docs_tokens > limited_token_nums:
                debug_logger.info(
        ...
        if second_file_dict:
            second_completed_doc, second_completed_doc_with_figure = (
                self.get_completed_document(second_file_dict["file_id"])
            )
            ...
            if first_doc_tokens + second_doc_tokens > limited_token_nums:
                if len(ori_second_docs) == 1:
                    ...
                    return new_docs
                doc_limit = [
                    min(second_file_dict["doc_ids"]),
                    max(second_file_dict["doc_ids"]),
                ]
                second_completed_doc_limit, second_completed_doc_limit_with_figure = (
                    self.get_completed_document(second_file_dict["file_id"], doc_limit)
                )
                second_completed_doc_limit.metadata["score"] = second_file_dict["score"]
                second_doc_tokens = custom_llm.num_tokens_from_docs(
                    [second_completed_doc_limit]
                )
                if first_doc_tokens + second_doc_tokens > limited_token_nums:
                    debug_logger.info(
    ...
    def incomplete_table(self, source_documents, limited_token_nums, custom_llm):
        ...
        existing_table_docs = [
            doc for doc in source_documents if doc.metadata.get("has_table", False)
        ]
        ...
        for doc in source_documents:
            ...
            if table_doc_id := doc.metadata.get("table_doc_id", None):
                ...
                current_table_docs = [
                    doc
                    for doc in source_documents
                    if doc.metadata.get("table_doc_id", None) == table_doc_id
                ]
                subtract_table_doc_tokens = custom_llm.num_tokens_from_docs(
                    current_table_docs
                )
                if (
                    current_doc_tokens + table_doc_tokens - subtract_table_doc_tokens
                    > limited_token_nums
                ):
                    debug_logger.info(
